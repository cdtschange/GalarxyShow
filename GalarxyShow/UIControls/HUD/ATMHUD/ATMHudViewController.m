...
#import "ATMHudViewController.h"
#import <GalarxyUIKitLib/ATMHud.h>
#import <GalarxyUIKitLib/ATMHudQueueItem.h>
...
@implementation ATMHudViewController
...
- (void)dealloc {
	...
	self.cellCaptions =nil;
	self.sectionHeaders =nil;
	self.sectionFooters =nil;
}
...
- (void)basicHudActionForRow:(NSUInteger)row {
	switch (row) {
		...
		case 2:
			...
			[hud setImage:[UIImage imageNamed:@"atmhud_check"]];
			break;
		...
		case 4:
			...
			[hud setShowSound:[[NSBundle mainBundle] pathForResource:@"atmhud_pop" ofType:@"wav"]];
			break;
	}
	[hud show];
}
...
- (void)advancedHudActionForRow:(NSUInteger)row {
	...
	switch (row) {
		...
		case 3: {
			...
			item.image = [UIImage imageNamed:@"atmhud_check"];
			item.accessoryPosition = ATMHudAccessoryPositionBottom;
			item.showActivity = NO;
			[hud addQueueItem:item];
...
- (void)positioningActionForRow:(NSUInteger)row {
	...
	switch (row) {
		...
		case 2:
			...
			[hud setImage:[UIImage imageNamed:@"atmhud_close"]];
			break;
...
- (void)hudDidAppear:(ATMHud *)_hud {
    NSString *soundFilePath = [[NSBundle mainBundle] pathForResource: @"atmhud_pop"
                                                              ofType: @"wav"];
