...
#import <GalarxyFoundationLib/ASIHTTPRequest.h>
#import "ASIDemoInfoCell.h"
#import "ASIDemoDetailCell.h"
...
{
	[super viewDidLoad];
	[self setTitle:@"HTTP Authentication"];
...
- (UITableViewCell *)tableView:(UITableView *)theTableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
	...
	if ([indexPath section] == 0) {
		...
		cell = [theTableView dequeueReusableCellWithIdentifier:@"InfoCell"];
		if (!cell) {
		...
		cell = [theTableView dequeueReusableCellWithIdentifier:@"ToggleCell"];
		if (!cell) {
			cell = [ASIDemoToggleCell cell];
			if ([indexPath row] == 0) {
				[[cell textLabel] setText:@"Use Keychain"];
				useKeychain = [(ASIDemoToggleCell *)cell toggle];
			} else {
				[[cell textLabel] setText:@"Use Built-In Dialog"];
				useBuiltInDialog = [(ASIDemoToggleCell *)cell toggle];
			}
		}

	} else {
		
		cell = [theTableView dequeueReusableCellWithIdentifier:@"Response"];
		if (!cell) {
...
{
	if ([indexPath section] == 0) {
		return [ASIDemoInfoCell neededHeightForDescription:intro withTableWidth:[theTableView frame].size.width]+20;
