...
#import "ASIDemoInfoCell.h"
#import <GalarxyFoundationLib/ASIWebPageRequest.h>
#import <GalarxyFoundationLib/ASIDownloadCache.h>
#import "ASIDemoToggleCell.h"
#import "ASIDemoRequestProgressCell.h"

// Private stuff
@interface ASIDemoWebPageViewController ()
- (void)webPageFetchFailed:(ASIHTTPRequest *)theRequest;
- (void)webPageFetchSucceeded:(ASIHTTPRequest *)theRequest;
@end

@implementation ASIDemoWebPageViewController
...
- (void)request:(ASIHTTPRequest *)theRequest didReceiveBytes:(long long)newLength
{
	...
	if (requestNumber != NSNotFound) {
		ASIDemoRequestProgressCell *cell = (ASIDemoRequestProgressCell *)[[self tableView] cellForRowAtIndexPath:[NSIndexPath indexPathForRow:requestNumber inSection:2]];
...
{
	[super viewDidLoad];
	[self setTitle:@"Downloading a Web Page"];
...
{
	int tablePadding = 40;
	int tableWidth = [self.tableView frame].size.width;
	...
	if ([indexPath section] == 0) {
		...
		if ([indexPath row] == 0) {
			cell = [self.tableView dequeueReusableCellWithIdentifier:@"WebPageCell"];
			if (!cell) {
			...
			cell = [self.tableView dequeueReusableCellWithIdentifier:@"ToggleCell"];
			if (!cell) {
				cell = [ASIDemoToggleCell cell];
			}
			[[cell textLabel] setText:@"Replace URLs with Data"];
			replaceURLsSwitch = [(ASIDemoToggleCell *)cell toggle];
		...
		cell = [self.tableView dequeueReusableCellWithIdentifier:@"RequestProgressCell"];
		if (!cell) {
			cell = [ASIDemoRequestProgressCell cell];
		}
		...
		if ([theRequest didUseCachedResponse]) {
			...
			if ([theRequest contentLength] > 0) {
				[[(ASIDemoRequestProgressCell *)cell progressView] setProgress:(float)(([theRequest totalBytesRead]*1.0)/(([theRequest contentLength]+[theRequest partialDownloadSize])*1.0))];
			}
		...
		cell = [self.tableView dequeueReusableCellWithIdentifier:@"Response"];
		if (!cell) {
...
{
	if ([indexPath section] == 0) {
		return [ASIDemoInfoCell neededHeightForDescription:intro withTableWidth:[self.tableView frame].size.width]+20;
	} else if ([indexPath section] == 1) {
		if ([indexPath row] == 0) {
			return 300;
		} else {
