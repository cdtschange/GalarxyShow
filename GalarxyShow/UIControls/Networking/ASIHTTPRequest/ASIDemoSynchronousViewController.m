...
@implementation ASIDemoSynchronousViewController
...
{
	[super viewDidLoad];
	[self setTitle:@"Synchronous Requests"];
...
- (UITableViewCell *)tableView:(UITableView *)theTableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
	...
	if ([indexPath section] == 0) {
		...
		cell = [self.tableView dequeueReusableCellWithIdentifier:@"URLCell"];
		if (!cell) {
		...
		cell = [self.tableView dequeueReusableCellWithIdentifier:@"ResponseCell"];
		if (!cell) {
		...
		cell = [self.tableView dequeueReusableCellWithIdentifier:@"HeaderCell"];
		if (!cell) {
			cell = [ASIDemoDetailCell cell];
		}
